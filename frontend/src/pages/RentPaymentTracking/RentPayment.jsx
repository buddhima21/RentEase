...
function CardTile({ card, selected, onSelect }) {
  return (
    <div
      className={`flex items-center justify-between p-4 mb-3 border-2 rounded-2xl cursor-pointer transition-all ${selected ? "border-primary bg-primary/5" : "border-slate-100 dark:border-slate-700/50 hover:border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900"
        }`}
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-[0.15em]">•••• •••• •••• {card.last4}</div>
          <div className="text-xl text-slate-500 dark:text-slate-400 font-bold mt-1">Expires {card.expiry}</div>
        </div>
      </div>
      <div className={`w-10 h-10 rounded-full border-4 flex items-center justify-center transition-all ${selected ? "border-primary bg-primary scale-110 shadow-lg shadow-primary/20" : "border-slate-300 dark:border-slate-600"
        }`}>
        {selected && <div className="w-4 h-4 rounded-full bg-white dark:bg-slate-900" />}
      </div>
    </div>
  );
}
...
export default function RentPayment({ bill, onComplete }) {
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-900 rounded-2xl border border-emerald-100 shadow-sm overflow-hidden">
      ...
      <div className="bg-primary/5 p-8 lg:p-12 border-b border-primary/10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
        <div>
          ...
          <div className="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tighter">{displayAmount}</div>
        </div>

        <div className="bg-white dark:bg-slate-900 px-6 py-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700/50 text-center">
      ...
      <div className="p-8 lg:p-12">
          ...
          <div className="mb-12">
            <h1 className="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white mb-4 tracking-tight">Bill Payment Portal</h1>
            <p className="text-lg lg:text-xl text-slate-500 dark:text-slate-400">{displayUnit} · {displayBillNo} · Premium Resident Portfolio</p>
          </div>
            ...
            <div className="flex gap-6">
              {[["card", "credit_card", "Debit / Credit Card"], ["bank", "account_balance", "Direct Bank Transfer"]].map(([id, icon, label]) => (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-4 py-6 lg:py-8 rounded-[2rem] transition-all font-black text-lg lg:text-xl ${method === id ? "bg-primary text-white shadow-[0_20px_50px_rgba(13,148,136,0.3)] scale-[1.02]" : "bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-800 border-2 border-transparent"
                    }`}
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            <div>
              ...
              <input
                className="w-full bg-slate-50 dark:bg-slate-800/50 border-4 border-slate-50 rounded-[1.5rem] px-8 py-5 lg:py-6 text-lg lg:text-xl focus:border-primary focus:bg-white dark:bg-slate-900 outline-none transition-all shadow-inner font-medium"
          ...
          {method === "card" && (
            <div className="mb-8">
              ...
              {savedCards.length === 0 ? (
                  <p className="text-slate-500 dark:text-slate-400 mb-4 text-sm ml-1 font-medium">No saved cards found. Please add a new card.</p>
              ) : (
                  savedCards.map(card => (
              ...
              {!showAddCard ? (
                <button
                  className="w-full py-4 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 text-slate-400 text-xs font-bold uppercase hover:border-primary/40 hover:text-primary transition-all flex items-center justify-center gap-2"
                  onClick={() => setShowAddCard(true)}
                >
                  <span className="material-symbols-outlined">add</span> Add New Payment Method
                </button>
              ) : (
                <div className="mt-8 p-12 bg-slate-50 dark:bg-slate-800/50 rounded-[3rem] border-2 border-slate-100 dark:border-slate-700/50 animate-in fade-in slide-in-from-top-4 duration-300">
                  <div className="flex justify-between items-center mb-8">
                    <span className="text-2xl font-black uppercase text-slate-900 dark:text-white tracking-widest">Add New Secure Card</span>
                    <button className="text-slate-400 hover:text-slate-600 dark:text-slate-300" onClick={() => setShowAddCard(false)}>
          ...
          {method === "bank" && (
            <div className="mb-14">
              ...
              <input 
                className="w-full bg-slate-50 dark:bg-slate-800/50 border-4 border-slate-50 rounded-[1.5rem] px-8 py-8 text-3xl font-black tracking-widest focus:border-primary focus:bg-white dark:bg-slate-900 outline-none shadow-inner" 
                name="bankAccount" 
        ...
        {!success && (
          <div className="bg-slate-50 dark:bg-slate-800/50 py-4 text-center border-t border-emerald-100">
        ...
        {success && (
          <div className="bg-emerald-50 p-8 border-t-2 border-emerald-200 text-center">
            ...
            <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-2">Payment Sent!</h2>
            <p className="text-slate-600 dark:text-slate-300 mb-6 max-w-sm mx-auto">
              {displayAmount} has been successfully paid for {displayUnit}. A receipt has been sent to <span className="text-slate-900 dark:text-white font-bold">{form.email}</span>.
            </p>
