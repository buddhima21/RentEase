                ...
                >
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {isEmergency ? (
                            ...
                            <span className="text-sm font-semibold text-slate-500">Standard requests can choose Low, Medium, or High priority.</span>
                        )}
                    </div>
                    ...
                    <form className="mt-8 space-y-4" onSubmit={submit}>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <select className="w-full rounded-xl border border-slate-300 p-3" value={form.serviceType} onChange={(e) => setForm({ ...form, serviceType: e.target.value })} required>
                            ...
                            {canChoosePriority ? (
                                ...
                                <input className="w-full rounded-xl border border-slate-200 bg-slate-100 p-3 text-slate-500" value="EMERGENCY" disabled readOnly />
                            )}
                        </div>
                        ...
                        {!isEmergencyPriority(form.priority) ? (
                            <div className="grid gap-4 md:grid-cols-2">
                                <input className="w-full rounded-xl border border-slate-300 p-3" type="date" min={today} aria-label="Preferred date" value={form.preferredDate} onChange={(e) => setForm({ ...form, preferredDate: e.target.value })} />
                                <input className="w-full rounded-xl border border-slate-300 p-3" type="time" aria-label="Preferred time" value={form.preferredTime} onChange={(e) => setForm({ ...form, preferredTime: e.target.value })} />
                            </div>
                        ) : null}

                        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                            <div className="flex items-center justify-between gap-4">
                                <p className="text-sm font-semibold text-slate-700">Photos ({form.imageUrls.length}/{MAX_MAINTENANCE_IMAGES})</p>
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3">
                                {form.imageUrls.map((img, idx) => (
                                    <div key={idx} className="relative overflow-hidden rounded-xl border border-slate-200 bg-white">
                                ...
                                {form.imageUrls.length === 0 ? <p className="text-sm text-slate-500">No photos attached yet.</p> : null}
                            </div>
                        </div>
