...
export default function OwnerFinance() {
    ...
    const [cardForm, setCardForm] = useState({ cardHolderName: "", cardNumber: "", expiryDate: "" });
    ...
    const fetchTenants = async () => {
        try {
            const response = await axios.get("http://localhost:8081/api/v1/users/tenants");
            if (response.data.success) {
                const mappedTenants = response.data.data.map(u => ({
                    id: u.id,
                    name: u.fullName,
                    email: u.email,
                    unit: "Unit TBD",
    ...
    const fetchInvoices = async () => {
        ...
        try {
            const response = await axios.get("http://localhost:8081/api/v1/invoices");
    ...
    const fetchWallet = async () => {
        ...
        try {
            const [wResp, tResp] = await Promise.all([
                axios.get(`http://localhost:8081/api/v1/wallet/${user.id}`),
                axios.get(`http://localhost:8081/api/v1/wallet/${user.id}/transactions`)
            ]);
    ...
    const handleSaveCard = async (e) => {
        ...
        try {
            ...
            if (cardForm.id) {
                await axios.put(`http://localhost:8081/api/v1/cards/${cardForm.id}`, payload);
            } else {
                await axios.post(`http://localhost:8081/api/v1/cards`, payload);
            }
    ...
    const handleWithdraw = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`http://localhost:8081/api/v1/wallet/${user.id}/withdraw`, { amount: amt, cardId: selectedCardForWithdrawal });
            if (response.data.success) {
    ...
    const handleDeleteInvoice = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:8081/api/v1/invoices/${id}/owner`);
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            const response = await fetch("http://localhost:8081/api/v1/invoices/generate-pdf", {
                method: "POST",
    ...
    const handleSendInvoice = async () => {
        ...
        try {
            await axios.post("http://localhost:8081/api/v1/invoices/send", billData);
                                ...
                                <div className="lg:col-span-2 bg-emerald-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-2xl shadow-emerald-200">
                                    <div className="relative z-10">
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <button onClick={() => {
                                                setCardForm({ id: "", cardHolderName: "", cardNumber: "", expiryDate: "" });
                                    ...
                                    <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
                                        ...
                                        <form onSubmit={handleSaveCard} className="space-y-4">
                                            <div className="space-y-1">
                                                ...
                                                <input required className={inputCls(false)} value={cardForm.cardHolderName} onChange={e => setCardForm({...cardForm, cardHolderName: e.target.value})} placeholder="JOHN DOE" />
                                            </div>
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Card Number</label>
                                                <input required className={inputCls(false)} value={cardForm.cardNumber} onChange={e => handleCardInput(e, 'cardNumber')} placeholder="xxxx xxxx xxxx xxxx" maxLength="19" />
                                            </div>
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Expiry Date</label>
                                                    <input required className={inputCls(false)} value={cardForm.expiryDate} onChange={e => handleCardInput(e, 'expiryDate')} placeholder="MM/YY" maxLength="5" />
                                                </div>
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-black uppercase text-slate-500 ml-1">CVV</label>
                                                    <input required className={inputCls(false)} placeholder="***" type="password" maxLength="3" onInput={(e) => { e.target.value = e.target.value.replace(/\D/g, '') }} />
                                                </div>
                                            </div>
