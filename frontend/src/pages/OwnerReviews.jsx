...
import { getOwnerProperties, getPropertyReviews, updateReviewStatus, replyToReview } from "../services/api";
...
export default function OwnerReviews() {
    ...
    const fetchReviews = async () => {
        ...
        try {
            ...
            // Fetch reviews for each property (concurrently)
            const reviewPromises = propsList.map(p => getPropertyReviews(p.id, false));
            const reviewResponses = await Promise.all(reviewPromises);
            
            // Flatten the results
            let allReviews = [];
            reviewResponses.forEach(res => {
                allReviews = [...allReviews, ...res.data.data];
            });
