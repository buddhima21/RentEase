...
export default function ActionDropdown({ onUpdate, onDelete, onView }) {
    ...
    const MenuItem = ({ icon, label, onClick, isDestructive }) => (
        <button
            ...
            className={`w-full text-left px-5 py-3 text-sm font-semibold flex items-center gap-3 transition-colors duration-200 ${
                isDestructive 
    ...
    return (
        <div className="relative inline-block text-left" ref={dropdownRef}>
            ...
            <button
                ...
                className={`p-2 rounded-xl transition-all duration-200 flex items-center justify-center border ${
                    isOpen 
                        ...
                        : "text-slate-400 bg-white border-transparent hover:text-primary hover:bg-slate-50 hover:border-slate-200 hover:shadow-sm"
                }`}
                aria-haspopup="true"
            ...
            {/* Dropdown Menu */}
            <div 
                className={`absolute right-0 mt-2 w-48 bg-white/95 backdrop-blur-md rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] border border-slate-100 z-50 overflow-hidden transform origin-top-right transition-all duration-200 ${
