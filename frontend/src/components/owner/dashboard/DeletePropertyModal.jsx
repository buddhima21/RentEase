...
export default function DeletePropertyModal({ isOpen, onClose, onConfirm, propertyName }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all overflow-y-auto">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative animate-in fade-in zoom-in-95 duration-200 m-auto">
                <div className="p-6">
                    <div className="flex items-center justify-center w-12 h-12 mx-auto bg-red-100 rounded-full mb-4">
                        <span className="material-symbols-outlined text-red-600">warning</span>
                    </div>
                    
                    <h3 className="text-xl font-bold text-center text-slate-900 dark:text-white mb-2">Request Property Deletion</h3>
                    
                    <p className="text-center text-slate-500 dark:text-slate-400 text-sm mb-6">
                        You are about to request deletion for <span className="font-bold text-slate-900 dark:text-white">{propertyName}</span>.
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <select
                                ...
                                className="w-full p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-red-500 outline-none transition-all"
                            >
                        ...
                        {reason === "Other" && (
                            <div className="animate-in slide-in-from-top-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-red-500 outline-none transition-all resize-none h-24"
                                />
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex gap-3 mt-8">
                        <button
                            onClick={onClose}
                            className="flex-1 py-3 px-4 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold rounded-xl hover:bg-slate-200 transition-colors"
                            disabled={isDeleting}
                        >
                            Cancel
                        </button>
