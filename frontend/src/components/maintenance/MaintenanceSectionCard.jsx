export default function MaintenanceSectionCard({ eyebrow, title, description, children, className = "" }) {
    return (
        <section className={`rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8 ${className}`}>
            {eyebrow ? <p className="text-xs font-bold uppercase tracking-[0.28em] text-emerald-600">{eyebrow}</p> : null}
            <h2 className="mt-3 text-2xl font-black tracking-tight text-slate-900 md:text-3xl">{title}</h2>
