...
public class ReviewService {
    ...
    public ReviewResponse createReview(ReviewRequest request, String userId) {
        log.info("Creating review for property: {} by reviewer: {}", request.getPropertyId(), userId);
    ...
    public List<ReviewResponse> getReviewsByProperty(String propertyId, boolean onlyApproved, String currentUserId) {
    ...
    // --- Helper Method ---
    public List<ReviewResponse> getReviewsForOwner(String ownerId) {
        log.info("Fetching all reviews for owner: {}", ownerId);
        
        // 1. Get all real properties for this owner
        List<String> propertyIds = new ArrayList<>(propertyRepository.findByOwnerIdAndDeletedFalse(ownerId).stream()
                .map(com.rentease.modules.property.model.Property::getId)
                .collect(Collectors.toList()));
        
        // 2. Add mock property IDs for testing/dev visibility
        // This ensures the owner can see reviews submitted for "Skyline Studio", etc.
        for (int i = 1; i <= 27; i++) {
            propertyIds.add("property_" + i);
        }
        
        log.info("Fetching reviews for {} property IDs", propertyIds.size());
